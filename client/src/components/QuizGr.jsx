...
const QuizGr = (props) => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:3001/dsatracker/quiz/graph/${username}`);
    ...
    const fetchBestScore = async () => {
        try{
            const rsp = await fetch(`http://localhost:3001/dsatracker/quizscore/graph/${username}`);
    ...
    useEffect(() => {
        const sendBestScoreToBackend = async () => {
            try {
                await axios.post(`http://localhost:3001/dsatracker/setbestscore/graph/${props.username}`, { bestscore });
        ...
        <div className='gridbox'>
            <div className='leftdiv'>
                <div className='infodiv'>
                    <img src={graph} className='headimage'/>
                    <p className='topic'>Topic : Graph<br/>Score: {score}<br/>Best Score : {bestscore}</p><br/>
