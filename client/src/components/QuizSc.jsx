...
const QuizSc = (props) => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:3001/dsatracker/quiz/searching/${username}`);
    ...
    const fetchBestScore = async () => {
        try{
            const rsp = await fetch(`http://localhost:3001/dsatracker/quizscore/searching/${username}`);
    ...
    useEffect(() => {
        const sendBestScoreToBackend = async () => {
            try {
                await axios.post(`http://localhost:3001/dsatracker/setbestscore/searching/${props.username}`, { bestscore });
        ...
        <div className='gridbox'>
            <div className='leftdiv'>
                <div className='infodiv'>
                    <img src={searching} className='headimage'/>
                    <p className='topic'>Topic : Searching<br/>Score: {score}<br/>Best Score : {bestscore}</p><br/>
