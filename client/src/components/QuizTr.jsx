...
const QuizTr = (props) => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:3001/dsatracker/quiz/trees/${username}`);
    ...
    const fetchBestScore = async () => {
        try{
            const rsp = await fetch(`http://localhost:3001/dsatracker/quizscore/trees/${username}`);
    ...
    useEffect(() => {
        const sendBestScoreToBackend = async () => {
            try {
                await axios.post(`http://localhost:3001/dsatracker/setbestscore/trees/${props.username}`, { bestscore });
        ...
        <div className='gridbox'>
            <div className='leftdiv'>
                <div className='infodiv'>
                    <img src={trees} className='headimage'/>
                    <p className='topic'>Topic : Trees<br/>Score: {score}<br/>Best Score : {bestscore}</p><br/>
