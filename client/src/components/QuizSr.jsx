...
const QuizSr = (props) => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:3001/dsatracker/quiz/strings/${username}`);
    ...
    const fetchBestScore = async () => {
        try{
            const rsp = await fetch(`http://localhost:3001/dsatracker/quizscore/strings/${username}`);
    ...
    useEffect(() => {
        const sendBestScoreToBackend = async () => {
            try {
                await axios.post(`http://localhost:3001/dsatracker/setbestscore/strings/${props.username}`, { bestscore });
        ...
        <div className='gridbox'>
            <div className='leftdiv'>
                <div className='infodiv'>
                    <img src={string} className='headimage'/>
                    <p className='topic'>Topic : String<br/>Score: {score}<br/>Best Score : {bestscore}</p><br/>
