...
const QuizSk = (props) => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:3001/dsatracker/quiz/stack/${username}`);
    ...
    const fetchBestScore = async () => {
        try{
            const rsp = await fetch(`http://localhost:3001/dsatracker/quizscore/stack/${username}`);
    ...
    useEffect(() => {
        const sendBestScoreToBackend = async () => {
            try {
                await axios.post(`http://localhost:3001/dsatracker/setbestscore/stack/${props.username}`, { bestscore });
        ...
        <div className='gridbox'>
            <div className='leftdiv'>
                <div className='infodiv'>
                    <img src={stack} className='headimage'/>
                    <p className='topic'>Topic : Stack<br/>Score: {score}<br/>Best Score : {bestscore}</p><br/>
