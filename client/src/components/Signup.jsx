...
function Signup(){
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(user.username && user.email && user.password && (user.password === user.reEnterPassword)){
            axios.post("http://localhost:3001/dsatracker/signup", user)
            .then(res => {
        ...
        <div className="containerSg">
            <form onSubmit={handleSubmit}>
                ...
                <div className="confirm box">
                    <input type="password" placeholder="Confirm Password" name="reEnterPassword" value={user.reEnterPassword} onChange={handleChange} required/>
                    <i className="fa-solid fa-lock"></i>
                </div>
                <button className="btn" type='submit'>SignUp</button>
                <div className="register">
                    <p>Already have an account? <Link to='/dsatracker/login'>Login</Link></p>
                </div>
            </form>
        </div>
